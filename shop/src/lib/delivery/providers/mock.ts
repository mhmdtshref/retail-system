import { Provider, CreateShipmentResult, DeliveryStatus } from '../index';
import { mockDb } from '@/lib/mock/store';

// In-memory mock provider lifecycle map
const g = globalThis as unknown as { __mockShipments?: Map<string, { status: DeliveryStatus; createdAt: number }> };
if (!g.__mockShipments) g.__mockShipments = new Map();
const mem = g.__mockShipments;

function nextStatus(cur: DeliveryStatus): DeliveryStatus {
  const flow: DeliveryStatus[] = ['created','in_transit','out_for_delivery','delivered'];
...
export const provider: Provider = {
  ...
  async getStatus(externalId: string): Promise<{ status: DeliveryStatus; payload?: any }> {
