...
import { PosCartLine, PosPayment } from '@/lib/pos/types';
...
type Actions = {
  ...
  addPayment: (method: PosPayment['method'], amount: number) => Promise<void>;
};
...
export const usePosStore = create<State & Actions>((set, get) => ({
  ...
  addPayment: async (method, amount) => {
    ...
    set({ payments: [...s.payments, payment] });
  },
}));
