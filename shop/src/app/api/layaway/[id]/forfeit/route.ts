...
import { LayawayForfeitSchema } from '@/lib/validators/layaway';
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const parsed = LayawayForfeitSchema.safeParse(body);
  ...
  if (lay.status === 'completed' || lay.status === 'forfeited' || lay.status === 'canceled') {
    return NextResponse.json({ error: { message: 'لا يمكن مصادرة هذه الحالة' } }, { status: 400 });
  }
  lay.status = 'forfeited';
  lay.forfeitedAt = new Date().toISOString();
  await lay.save();
  // TODO: audit log forfeit
