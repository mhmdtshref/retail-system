...
import { forfeitLayaway } from '@/lib/layaway/service';
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const layaway = await forfeitLayaway(id);
    try { await AuditLog.create({ action: 'layaway.forfeited', subject: { type: 'Layaway', id: String(layaway._id) }, actorUserId: (auth as any).user?.id }); } catch {}
    const out = { layaway };
